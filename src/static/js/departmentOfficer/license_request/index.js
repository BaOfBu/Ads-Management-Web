$(document).ready(function() {
    ...
    function populateWardsDropdown(wards) {
        ...
        dropdownWard.append(`<li><a href="license-request?districtId=${districtId}&wardId=-1&page=1" class="dropdown-item active" data-value="Tất cả phường" data-id="-1">Tất cả phường</a></li>`);
        for (let i = 0; i < wards.length; i++) {
            let ward = wards[i];
            dropdownWard.append(`<li><a href="license-request?districtId=${districtId}&wardId=${ward.wardId}&page=1" class="dropdown-item" data-value="` + ward.name + '" data-id="' + ward.wardId + '">Phường ' + ward.name + '</a></li>');
        }
    }
